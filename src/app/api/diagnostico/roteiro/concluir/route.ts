/**
 * Recebe o roteiro pronto do worker, escreve no card do evento e fecha a fila.
 *
 ...
 *
 * Ordem deliberada: escreve no Google PRIMEIRO, marca concluído depois. Se a
 * escrita falhar, o item continua na fila e é tentado de novo. O inverso
 * deixaria a fila dizendo "pronto" com o card do evento vazio, que é o tipo de
...
import { acharEvento, ErroAgenda, escreverDescricao } from "@/lib/diagnostico/agenda-google";
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const eventoId = corpo.googleEventId ?? (await acharEvento(calBookingId, inicio, email));
    await escreverDescricao(eventoId, descricao);
